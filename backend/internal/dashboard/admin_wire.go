...
func AdminAPIPaths() []AdminAPIPath {
	...
	return []AdminAPIPath{
		...
		{Method: "GET", Path: "/admin/api/logs/rollup", OperationID: "getLogsRollup", Summary: "Request-outcome rollup: counts, group-bys, error rate, TTFB percentiles, time buckets", Auth: "sensitive", Kind: AdminAPIKindJSON, Response: logsRollupData{}, Query: []AdminAPIQuery{{Name: "since", Description: "Unix-millis lower bound (default 0, clamped to 168h)"}, {Name: "until", Description: "Unix-millis upper bound (default now)"}, {Name: "bucket_ms", Description: "Time-bucket width in millis (default 300000)"}}},
