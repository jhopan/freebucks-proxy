...
The scenarios in §3/§4 are the **pre-#666 baseline** (fork 1.12.1.10). After
merging upstream `v1.13.0` (`e181d281`, *global per-model smart queue*) and
deploying fork `1.13.0.18`, three things changed measurably.

### 10.1 Arrival converges on the warm lane

`smartAcquire` (`pool/acquire_route.go`) first runs `scanWarmFree`: the first
lane in index order that (a) passes the gates, (b) already holds a usable
session for the model, and (c) has a free permit, grants **instantly**. Only on
a miss does the walk (`scaleoutFrom`) admit on a free cold lane (creating its
session inline) or park on the full one — and the code comment states the
intent: *"walkers stick to the admitting lane instead of spreading, so a burst
converges onto one lane and extra accounts stay cold"*.

Measured on the 3-account pool (`SLOTS_PER_ACCOUNT=3`): every probe wave served
from the single warm lane (`slot1`), with `slot0`/`slot2` untouched and their
sessions never created.

### 10.2 Rejection now costs ONE wait budget

A request that cannot get a permit parks once and then fails over without
parking again (`park=false`, *"the single QUEUE_WAIT deadline is never paid
twice"*). Measured: rejections surface at **2.0 s** with `QUEUE_WAIT=2s`, where
the pre-#666 build needed 4.0 s (2 s per lane walked).

### 10.3 Deepseek is metered now — the gate that actually binds

`deepseek/deepseek-v4-flash` is **no longer an unpriced row** (the 2026-09-08
cost-0 observation is stale). Verified through `GET /admin/api/tokens`:

```
slot1 prices: {"deepseek/deepseek-v4-flash": 5, "crof/kimi-k3-eco": 0,
               "google/gemini-3.8-flash": 40, ...}
slot1 daily : {"limit": 100, "spent": 15, "remaining": 85,
               "reset_at": "2026-09-21T00:00:00Z"}
```

`pool/quota.go:freebucksCappedForSnapshot` gates the walk on that meter:

- a lane holding a **live reusable session for the model is never capped**
  (reuse performs no admission POST),
- otherwise the lane is skipped when `fb.Spendable() < price`,
- the two slots without a freebucks snapshot served no traffic at all while the
  funded slot served every request, and the refusals came back as the
  freebucks-shaped 429 (`upstream rate limited (reset at 2026-09-21T00:00:00Z)`).

Operational consequences:

| Fact | Effect |
|---|---|
| one session costs 5 Freebucks, daily allowance 100 | ~20 new sessions per account per day |
| the meter is charged per **session**, not per request | keep sessions alive; more concurrent turns on one lane (`SLOTS_PER_ACCOUNT`) is cheaper than extra lanes |
| an exhausted lane is skipped, not spilled around | pool capacity for the day is effectively "accounts with remaining allowance" |
| sessions reset at 00:00 UTC (07:00 WIB) | capacity returns daily, not continuously |

So the capacity formula in §5 needs a qualifier: `accounts with remaining
freebucks (or a live session) x SLOTS_PER_ACCOUNT` — a lane whose allowance is
spent is invisible to the walk until the reset.
